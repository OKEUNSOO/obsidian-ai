...
export class SessionStorage {
  private _provider: 'claude' | 'codex' | null = null;

  constructor(private adapter: VaultFileAdapter) {}

  /** Set the provider so all session paths go into a provider subdirectory. */
  setProvider(provider: 'claude' | 'codex'): void {
    this._provider = provider;
  }

  /** Get the sessions directory, provider-aware. */
  private getSessionsDir(): string {
    return this._provider ? `${SESSIONS_PATH}/${this._provider}` : SESSIONS_PATH;
  }
  ...
  async listConversations(): Promise<ConversationMeta[]> {
    ...
    try {
      const files = await this.adapter.listFiles(this.getSessionsDir());
  ...
  getFilePath(id: string): string {
    return `${this.getSessionsDir()}/${id}.jsonl`;
  }
  ...
  /**
   * Migrate existing flat sessions to provider subdirectories.
   * Copies `.claude/sessions/*.jsonl` → `.claude/sessions/claude/`.
   * No-op if claude subdir already exists or no JSONL files are found.
   */
  async migrateToProviderDirs(adapter: VaultFileAdapter): Promise<void> {
    const claudeDir = `${SESSIONS_PATH}/claude`;
    const claudeDirExists = await adapter.exists(claudeDir);
    if (claudeDirExists) return; // already migrated

    const files = await adapter.listFiles(SESSIONS_PATH);
    const jsonlFiles = files.filter(f => f.endsWith('.jsonl'));
    if (jsonlFiles.length === 0) return; // nothing to migrate

    await adapter.ensureFolder(claudeDir);
    for (const file of jsonlFiles) {
      const filename = file.split('/').pop()!;
      const content = await adapter.read(file);
      await adapter.write(`${claudeDir}/${filename}`, content);
    }
  }

...
import type { ImageMode, VisualOutputType } from '../../core/images/ImagePromptBuilder';
...
export class ImageGenerationModal extends Modal {
  ...
  onOpen(): void {
    ...
    contentEl.empty();
    contentEl.createEl('h2', { text: '이미지 생성' });
    contentEl.createEl('p', {
      text: '현재 노트를 분석해 이미지 프롬프트를 만들고, Codex CLI로 시각 자료를 생성한 뒤 노트 상단에 삽입합니다. 별도 API 키는 필요하지 않습니다.',
    });

    new Setting(contentEl)
      .setName('출력')
      .setDesc('Codex 내장 PNG 생성 또는 텍스트 기반 SVG 생성을 선택하세요.')
      .addDropdown((dropdown) => {
        dropdown
          .addOption('png', 'Codex 이미지 생성으로 PNG 만들기')
          .addOption('svg', 'Codex 코드 생성으로 SVG 만들기')
    ...
    new Setting(contentEl)
      .setName('방향')
      .setDesc('스타일, 대상 독자, 레이아웃, 포함할 내용 등을 선택적으로 적어주세요.')
      .addTextArea((text) => {
        text
          .setPlaceholder('뉴스레터 헤더에 어울리게 만들고, 한글 라벨은 짧고 명확하게 써줘...')
          .onChange((value) => {
            this.prompt = value;
          });
        text.inputEl.rows = 6;
